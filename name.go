...
func (c context) FileName(node pgs.Node, suffix string) pgs.Name {
	e, ok := node.(pgs.File)
	if !ok {
		return ""
	}
	if e.Descriptor().Name == nil {
		return pgs.Name(suffix)
	}

	name := path.Base(*e.Descriptor().Name)
	return pgs.Name(name[:len(name)-len(path.Ext(name))] + suffix)
}
